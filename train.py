...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        for i, (text, mask, age, label) in enumerate(progress_bar):
            ...
            # Get the predicted class and actual label
            pred_class = torch.argmax(pred, dim=1)  # Get the predicted class

            # Log predictions and labels to file
            with open(f'log/{language_model}_train_pred_vs_label.txt', mode='a') as n:
                n.write(f'{time.asctime(time.localtime(time.time()))}\n')
                
                # Convert tensors to CPU and iterate through the batch
                for p, l in zip(pred_class.cpu().numpy(), label.cpu().numpy()):
                    n.write(f'Prediction: {p} || Label: {l}\n')
